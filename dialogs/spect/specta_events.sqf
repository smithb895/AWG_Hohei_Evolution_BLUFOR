...
switch (_type) do {
	...
	case "MapClick": {
		...
		_idxx = 0;
		{
			if ((side (group _x)) in KEGs_ShownSides) then {
	...
	case "UnitKilled": {
		...
		_txt = format["%1 (%2) was killed by %3 (%4) (%5m)", _killed, _killed call KEGs_CheckOriginalSide, _killer, side (group _killer), _killed distance _killer];
