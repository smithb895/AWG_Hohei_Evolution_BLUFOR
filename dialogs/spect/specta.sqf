...
if (isNil "KEGs_FuncsInited") then {
	...
	call compile preprocessFileLineNumbers "dialogs\spect\spect_funcs.sqf";
	spectate_events = compile preprocessFileLineNumbers "dialogs\spect\specta_events.sqf";
	KEGs_menucamslbchange = compile preprocessFileLineNumbers "dialogs\spect\menucamslbchange.sqf";
};
...
while {dialog} do {
	...
	if (KEGs_DroppedCamera) then {
		...
		_vpmtw =
		#ifndef __A2__
			visiblePosition (vehicle KEGs_target);
		#else
			((vehicle KEGs_target) modelToWorld [0, 0, 0]);
		#endif
...
if (KEGs_no_butterfly_mode) exitWith {
	...
	[] spawn {sleep 0.01; [_player, _killer, "noWait"] execVM "dialogs\spect\specta.sqf"};
};
...
deletevehicle _bird;
KEGs_SPECTATINGON = nil;
[_player, _killer, "noWait"] execVM "dialogs\spect\specta.sqf";