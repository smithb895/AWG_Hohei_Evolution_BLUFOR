...
if (!(_SelectedTransporterTypeS) && !(_SelectedTransporterTypeM) && !(_SelectedTransporterTypeL) && !(_SelectedTransporterTypeXL)) exitWith {
	...
	hint parseText format ["
		...
		<img color='#ffffff' image='Common\Cargo_Drop\Images\img_line_ca.paa' align='left' size='0.79' />
...
if ((_Selected == "LoadCargo") && (_SelectedTransporterTypeS)) exitWith {
	...
	_UnloadAction = _Transporter addAction [("<t color=""#fadfbe"">" + ("Unload cargo") + "</t>"),"Common\Cargo_Drop\Engine.sqf",["UnloadCargo"], 5];
