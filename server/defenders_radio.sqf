...
_radio_position = _this select 0;
_grp = [EVO_enemy_side] call EVO_createGroup;
_enemy_list = [];

_lead = EVO_e_leader select (round random (count EVO_e_leader - 1));
for "_i" from 0 to 9 do {
	_lucky_one = EVO_e_soldiers select (round random (count EVO_e_soldiers - 1));
	_enemy_list set [count _enemy_list, _lucky_one];
};
_leader = _grp createUnit [_lead, _radio_position, [], 20,"NONE"];
_grp selectLeader _leader;
__addDead(_leader)

{
	_one_unit = _grp createUnit [_x, _radio_position, [], 20,"NONE"];
	_svec = sizeOf _x;
...
[_grp, _radio_position] spawn EVO_taskDefend;