...
while {true} do {
	...
	#endif
	{
		...
		if ((_disabled && _empty) || (_empty && !(alive _vehicle))) then {
			...
			_vehicle setVehicleInit format ["%1;", 'this addeventhandler ["fired",{  if (vehicle (_this select 0) in list base ) then {deleteVehicle (_this select 6)}; }];  this addAction ["HALO jump", "awg\halo.sqf"];'];
			processInitCommands;
